...
func (client *NodeClient) proxyConnection(incoming net.Conn, remoteAddr string) {
...
func (client *NodeClient) listenAndForward(socket net.Listener, remoteAddr string) {
	...
	for {
		...
		go c.proxyConnection(incoming, remoteAddr)
	}
}
...
func (c *NodeClient) dynamicListenAndForward(ln net.Listener) {
	defer ln.Close()
	defer c.Close()
	...
	for {
		...
		conn, err := ln.Accept()
		if err != nil {
			c.log.Error("Dynamic forwarding failed: %v.", err)
			break
		}
		defer conn.Close()
		...
		// address to proxy.
		remoteAddr, err = utils.SocksHandshake(conn)
		if err != nil {
			c.log.Error(err)
			break
		}
